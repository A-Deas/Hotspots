...
def construct_output_map_path(dataset, training_years):
    output_map_path = f'choose path'
    data_path = f'Clean Data/{dataset} rates.csv' 
    kal_path = f'Kalman Filter Trained on Less Data/Kalman Predictions ToLD/{dataset} Kalman preds trained on {training_years} years.csv'  
...
def plot_accuracy_map(dataset, training_years, shape, year, output_map_path):
    """ Plot and save the accuracy map """

    fig, main_ax = plt.subplots(figsize=(10, 5))
    title = f'{year} Accuracy Map for the {dataset} Kalmans trained on {training_years} Years'
    ...
    for dataset in FACTOR_LIST:
        for training_years in range(1,5):
            for year in range(2020, 2021):
                output_map_path, data_path, kal_path = construct_output_map_path(dataset, training_years)
