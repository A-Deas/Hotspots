...
def construct_output_paths(dataset, training_years):
...
def calculate_err_acc(dataset, training_years, data_df, kals_df):
    ...
    for year in range(2020, 2021):
        ...
        print(f'The {year} average error for {dataset} trained on {training_years} is {avg_err:.2f}.')
        print(f'The {year} maximum error for {dataset} trained on {training_years} is {max_err:.2f}.')
        ...
        #print(f'The {year} average accuracy for {dataset} trained on {training_years} is {avg_acc * 100:.2f}%.\n')

def calculate_hotspot_acc(dataset, training_years, data_df, kals_df):
    ...
    for year in range(2020, 2021):
        ...
        print(f'The {year} hotspot accuracy for {dataset} trained on {training_years} is {hotspot_acc}%.')
...
def main():
    for dataset in ['OD','DR','SVI Disability']:
        ...
        for training_years in range(1,5):
            print()
            data_path, kalman_path = construct_output_paths(dataset, training_years)
            data_df, kals_df = load_data(data_path, DATA_NAMES, kalman_path, KALMAN_NAMES)
            #calculate_err_acc(dataset, training_years, data_df, kals_df)
            calculate_hotspot_acc(dataset, training_years, data_df, kals_df)
